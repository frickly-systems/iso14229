...
bazel coverage -c opt --config=hermetic-coverage //fuzz:fuzz_server_proto -- \
  "${CORPUS}/" \
  "${SEED_BIN}/"

# Copy and process the coverage report
cp -f "$(bazel info output_path)/_coverage/_coverage_report_proto.dat" coverage_proto.lcov
